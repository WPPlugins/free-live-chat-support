...
function zupport_generateAcctPage(){
	...
	if($_GET['BackToCompany']!=0 && trim($_POST['Email'])=='' && trim($_POST['selectedCompanyId'])=='' && trim($_POST['selectedWeigetID'])==''){
		update_option('zupportdesk_authorizeToken', '');
		update_option('zupportdesk_selectedCompanyId', '');
		update_option('zupportdesk_widgetId', '');
	}

	if(trim($_POST['selectedWeigetID'])!=''){
		$selectedWeigetID=sanitize_text_field($_POST['selectedWeigetID']);
		update_option('zupportdesk_widgetId', $selectedWeigetID);
	}
...
<div class="zupport-main-div">
	...
	// logout
	if($_GET['LogOut']!=0 && trim($_POST['Email'])=='' && trim($_POST['selectedCompanyId'])=='' && trim($_POST['selectedWeigetID'])==''){
		zupportDeactive();
	}
	if ($_GET['BackToCompany']!=0 && $_GET['LogOut']!=0 && trim($_POST['Email'])=='' && trim($_POST['Password'])=='' && trim($_POST['selectedCompanyId'])=='' && trim($_POST['selectedWeigetID'])=='') {
...
<?php }
	if (trim($_POST['selectedCompanyId'])!='' || isset($comp)) {
		...
		$getAuthorizetoken=file_get_contents($zupportCreateLoginToken.'?userToken='.$userId.'&UserCompanyID='.$selectedCompanyId);
		$authInArray=json_decode($getAuthorizetoken);
		$AuthorizeToken=$authInArray->ReturnObj;
...
<?php 	}
	...
	{ 
		...
		$data=array("Authorization"=>"Bearer ".$AuthorizeToken);
		$content = json_encode($data);
 
		$curl = curl_init($url);
		curl_setopt($curl, CURLOPT_HEADER, false);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($curl, CURLOPT_HTTPHEADER,array("Content-type: application/json","Authorization: Bearer ".$AuthorizeToken));
		...
		if($widget_response==null){
			// new authorise token
			$selectedCompanyId=urlencode(get_option('zupportdesk_selectedCompanyId'));
			$userId=urlencode(get_option('zupportdesk_userId'));
			$getAuthorizetoken=file_get_contents($zupportCreateLoginToken.'?userToken='.$userId.'&UserCompanyID='.$selectedCompanyId);
			$authInArray=json_decode($getAuthorizetoken);
			$AuthorizeToken=$authInArray->ReturnObj;
			...
			curl_close($curl);
		}//print_r($widget_result);print_r($widget_response);
?>	<div class="zupport-main-div">
		<div class="zupport-main" id="widget_table">
			<a id="CloseLogout" href="?page=ZupportDesk&LogOut=1" title="Log Out">X</a>
			<h1 class="widget-img">Widget List</h1>
			<div class="full-width text-center hide" id="widget-error-message">
				<div class="zupport-error">Please select widget.</div>
			</div>
			<div class="full-width text-left">Please select a widget from following list.</div>
			<div class="full-width">
				<table class="table table-striped"  cellpadding="0">
					<tbody>
							<?php foreach($widget_response as $widget_item ):?>
							<tr class="table_tr_weidget <?php if($widgetId==$widget_item->WidgetID){echo 'arrow-right';}?>" id="<?php echo $widget_item->WidgetID;?>">
								<td class="filterable-cell"><?php echo $widget_item->WidgetName;?></td>
							</tr>
							<?php endforeach;?>
					</tbody>
				</table>
				<form  action="?page=ZupportDesk" title="" method="post" id="WidgetSelect">
					<input type="hidden" name="selectedWeigetID" id="selectedWeigetID" value="<?php if($widgetId!=null){echo $widgetId;}?>"/>
					<a href="?page=ZupportDesk&BackToCompany=1"><button type="button" id="zupport_cancel">Back To Company List</button></a>
					<button type="submit" id="zupport_select">Save</button>
				</form>
			</div>
		</div>
	</div>
